-- From https://github.com/tony/awesome-config
-- {{{ Helper functions
...
function run_once(prg,arg_string,pname,screen)
	...
	if not arg_string then 
		awful.util.spawn_with_shell("pgrep -x '" .. pname .. "' || " .. prg .. "",screen)
	else
		awful.util.spawn_with_shell("pgrep -f -x '" .. pname .. "' || " .. prg .. " " .. arg_string .. "",screen)
	end
end
...
function command_exists(file)
	local cmd = "/bin/bash -c 'hash " .. file .. " && echo true'"
	local fh = io.popen(cmd)
